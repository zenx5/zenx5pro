...
interface ItemProjectProps extends ProjectType {
...
export default function ItemProject(props:ItemProjectProps){
    ...
    useEffect(()=>{
        function checkViewport() {
            ...
            setShow( props?.defaultVisibility as boolean )
            if( width ) {
    ...
    return <div className="relative flex flex-col justify-between gap-0 border border-slate-200 rounded-lg shadow-lg overflow-hidden w-full h-fit group text-black bg-white pt-0 sm:pt-2 sm:h-72 sm:gap-2">
        {enableShow && <button className="absolute right-5 top-2 w-4 h-4" onClick={handlerShow}>
            { show ?
